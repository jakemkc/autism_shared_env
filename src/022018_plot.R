...
# write_csv(retFrame, path = file.path("results", "hilicEWASplot.csv"))
...
retFrame_1 <- retFrame_1 %>% filter(p.value <=2.796174e-05) # top 4
tmp_index1 <- which(tablemz_sign_unique$mzid %in% retFrame_1$mzid)
retFrame_1 <- bind_cols(retFrame_1, tablemz_sign_unique[tmp_index1, "Name"])
p <- p + geom_text_repel(data = retFrame_1, aes(label = Name), nudge_x = 0.4,
                         size = 3,
                         box.padding = 1,
                         point.padding = 0.8)
