...
autism_sign %>% group_by(Role) %>% summarise(counttotal = n(),
                                           countuniqueID = n_distinct(ID),
                                           male = sum())

# # A tibble: 4 x 3
# Role    counttotal countuniqueID
# <chr>        <int>         <int>
# 1 Control         29            29
# 2 Father          44            39
# 3 Mother          42            40
...
## better way: get meta with unique count
basic <- autism_sign %>% select(-starts_with("v"))
basic <- basic %>% select(Batch, ID, Age, Sex, Role, Orig, Diag)
basic <- basic %>% distinct
basic %>% count(Role)

summary(basic)

basic %>% group_by(Role, Sex) %>% count
basic %>% group_by(Role, Orig) %>% count
