...
## 4.2) remove mz with no variance, per group (jake: this is not correlation but EWAS, if larger N and have time, dont do it per group to keep the rare exposure in case or control)
# F = no variant column
...
# tablemz_sign_unique <- readRDS("results/tablemz_sign_unique_hilic.rds") # from annotation.R
# 
...
# 
# tablemz_sign_unique <- readRDS("results/tablemz_sign_unique_hilic.rds")
# 
...
## special to ryan 012720
tmp1 <- autism_ccEwas %>% select(-starts_with("v"))
tmp2 <- autism_ccEwas %>% select(v6000)
tmp3 <- bind_cols(tmp1, tmp2)
write_csv(tmp3, path = file.path("results", "ryanv6000info.csv"))
