...
## save
outdirectory <- "./results"
outfilename <- sprintf("chemOrderClusTab_Mother_%s.rds", saveName)
...
## save
outdirectory <- "./results"
outfilename <- sprintf("chemOrderListASD_Mother_%s.rds", saveName)
...
mat_fm = as.matrix(cor_mother)

# hist(mat_fm)
###' start trial at rougly with 20% as cutoff

mat_fm[mat_fm < 0.45 & mat_fm > -0.45] <- NA  # set NA threshold
...
mat_f = as.matrix(cor_motherIntra)

mat_f[mat_f < 0.45 & mat_f > -0.45] <- NA  # set NA threshold
...
mat_m = as.matrix(cor_mothercaseIntra) 

mat_m[mat_m < 0.45 & mat_m > -0.45] <- NA  # set NA threshold
...
outdirectory <- "./results"
outfilename <- sprintf("chord_mother_%s.pdf", saveName)
